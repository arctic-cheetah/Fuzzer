...
class JSON_Mutational_Fuzzer(Fuzzer):
    def __init__(self, path_to_input: str, binary_path: str):
        ...
        self.mutators = [
            self.long_string, self.control_chars, self.break_quotes,
            self.trailing_commas, self.unbalanced_braces, self.duplicate_keys,
            self.deep_nesting, self.numeric_edgecases, self.random_block,
            self.shuffle_array_items, self.break_unicode, self.delete_random_block,
            self.repeat_token,
        ]
