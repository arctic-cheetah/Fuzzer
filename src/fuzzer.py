...
class Fuzzer:
    def __init__(self, path_to_input:str):
        self.corpus = open(path_to_input, 'rw') # doesn't have to be a file, can be loaded into memory, the
        self.mutators = [ 
            # add more as we make strategies
        ]
    def generate_rand_str(max_length:int = 100, char_start:int = 32, char_range:int = 32) -> str:
